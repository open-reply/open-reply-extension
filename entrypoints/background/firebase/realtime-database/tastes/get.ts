 ...
 */
export const _getUserTaste = async (): Promise<Returnable<Record<Topic, TopicTaste> | undefined, Error>> => {
  try {
 ...
 */
export const _getUserTopicTasteScore = async (topic: Topic): Promise<Returnable<number | undefined, Error>> => {
  try {
    const authCheckResult = await thoroughAuthCheck(auth.currentUser)
    if (!authCheckResult.status || !auth.currentUser) throw authCheckResult.payload
    ...
    logError({
      functionName: '_getUserTopicTasteScore',
      data: topic,
      error,
    })
