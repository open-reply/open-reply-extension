...
import { buttonVariants } from '@/entrypoints/content/components/ui/button'
...
const useShadowRootElement = () => {
  ...
  React.useEffect(() => {
    ...
    if (shadowHost && shadowHost.shadowRoot && applicationContainer) {
      // Create a div element to be used as a container if it doesn't exist
      if (!containerRef.current) {
        containerRef.current = document.createElement('div')
        containerRef.current.id = `alert-dialog-container-${idRef.current}`
      }
      
      // Append the container to the shadow root if it's not already there
      if (!shadowHost.shadowRoot.getElementById(`alert-dialog-container-${idRef.current}`)) {
        applicationContainer.appendChild(containerRef.current)
      }
...
const AlertDialogPortal: React.FC<AlertDialogPrimitive.AlertDialogPortalProps> = (props) => {
  ...
  return <AlertDialogPrimitive.Portal {...props} container={shadowRootElement} />
}
