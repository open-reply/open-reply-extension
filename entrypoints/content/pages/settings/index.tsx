...
const Settings = () => {
  ...
  const { isLoading: isAuthLoading, isAccountFullySetup, isSignedIn } = useAuth()
  const tabItems = [
    {
      title: 'Safety',
  ...
  useEffect(() => {
    if (!isAuthLoading && isSignedIn && isAccountFullySetup) navigate(ROUTES.SETUP_ACCOUNT)
  }, [isAuthLoading, isSignedIn, isAccountFullySetup])

  // Load the current user's preferences.
  useEffect(() => {
    if (!isAuthLoading && isAccountFullySetup && isSignedIn) loadUserPreferences()
  }, [isAuthLoading, isAccountFullySetup, isSignedIn])

  // Return:
  return (
    <main className='w-full pt-16 bg-white' style={{ height: 'calc(100% - 0px)' }}>
      <div className='h-full w-full flex flex-row'>
        <nav className='w-1/3 h-full flex flex-col pt-5 px-3 gap-2'>
          {tabItems.map((tab, index) => (
            <div
              className={cn('data-[state=active]:text-foreground data-[state=active]:shadow-sm data-[state=active]:bg-overlay transition-all', 'w-full p-3 flex flex-col gap-1 justify-start cursor-pointer bg-[#ffffff] hover:bg-overlay transition-colors duration-200 rounded-lg')}
              onClick={() => {
                setSelectedTab(index)
              }}
              data-state={selectedTab === index ? 'active' : 'inactive'}>
              <span className='font-semibold text-base'>{tab.title}</span>
              <span className='font-normal text-xs text-brand-secondary'>{tab.description}</span>
              <span className='font-normal text-[0.6rem] text-brand-tertiary'>{tab.quickView.join(' • ')}</span>
            </div>
          ))}
        </nav>
